...
def master(m: int, data: np.ndarray, decoder: None, G: np.ndarray):
    """
    ...
    nodes_array = np.zeros(m)
    width = data.shape[1]/m # number of cols of data stored at each node
    for i in range(m):
        # use most intuitive partition scheme
        nodes_array[i] = node(data[:,i*width:i*(width +1)], decoder, G)
